...
//
// TEMPORAERE Hilfsfunktion (nicht Teil der offiziellen Skill-Kette):
// erstellt eine Kopie eines bestehenden Klaviyo-Templates ueber Klaviyos
// Clone-Endpoint, damit Skill-A/B-Experimente (Marker einfuegen, HTML per
// API ueberschreiben) NICHT am Original-Template des Kunden getestet
// werden muessen. Nur POST /clone, keine sonstige Schreiboperation.
...
exports.handler = async (event) => {
  ...
  const neuerName = String(data.neuerName || '').trim() || (templateName + ' - TESTKOPIE');
  ...
  try {
    ...
    const cloneResult = await klaviyo.klaviyoRequest(
      klaviyoAccessToken,
      'POST',
      '/api/template-clone/',
      { data: { type: 'template', attributes: { id: tpl.id, name: neuerName } } }
    );
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        originalId: tpl.id,
        originalName: tpl.attributes.name,
        editor_type: tpl.attributes.editor_type,
        cloneId: cloneResult.data.id,
        cloneName: cloneResult.data.attributes.name,
        cloneEditorType: cloneResult.data.attributes.editor_type,
        klaviyoTemplateUrl: 'https://www.klaviyo.com/email-templates/' + cloneResult.data.id + '/edit',
      }),
    };
  } catch (err) {
    console.error(err);
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Fehler beim Klonen des Templates.', details: err.message }) };
  }
};
